...
Single source of truth for all FixMyCert content tracking.
Replaces the fixmycerttracker spreadsheet completely.

Transport: stdio (local only)
Storage: JSON files in ~/.fixmycert-content/
"""
...
DATA_DIR = Path(os.environ.get("DATA_DIR", str(Path.home() / ".fixmycert-content")))
...
VALID_CONTENT_TYPES = [
    "guide", "demo", "checklist", "tool", "cheatsheet", "blog", "governance",
]

VALID_CATEGORIES = [
    "compliance", "fundamentals", "certificates", "troubleshooting",
    "enterprise", "openssl", "java", "cdn", "email", "web-servers",
    "f5", "cloud", "governance", "tools", "enhancement", "signatures",
    "ssh", "operations", "adcs", "venafi", "security", "pki-disasters",
    "pki-mistakes",
]
...
VALID_CONTENT_STATUSES = ["live", "draft", "planned"]
...
def _fmt_content_short(item: dict) -> str:
    video = "🎬" if item.get("youtube_url") else "  "
    return f"{video} [{item['content_type']:10}] [{item['category']:15}] {item['title']}"
...
class RegisterContentInput(BaseModel):
    ...
    content_type: str = Field(..., description="Type: guide, demo, checklist, tool, cheatsheet, blog, governance")
    category: str = Field(..., description="Category: compliance, fundamentals, certificates, troubleshooting, enterprise, openssl, java, cdn, email, web-servers, f5, cloud, governance, tools, ssh, operations, etc.")
    path: str = Field(..., description="FixMyCert URL path (e.g., '/guides/dns-01-automation')")
    youtube_url: Optional[str] = Field(default=None, description="YouTube video URL if one exists")
    status: Optional[str] = Field(default="live", description="Status: live, draft, planned")
...
class AddBacklogInput(BaseModel):
    ...
    content_type: str = Field(..., description="Type: guide, demo, checklist, tool, etc.")
...
class AddPartnerInput(BaseModel):
    ...
    partnership_type: Optional[str] = Field(default=None, description="enterprise-referral, content-collaboration, affiliate-split, consulting-referral")
...
@mcp.tool(name="ct_register_content")
async def register_content(params: RegisterContentInput) -> str:
    """Register a new content piece (guide, demo, tool, checklist, etc.).
...
async def list_content(params: ListContentInput) -> str:
    ...
    Use to answer 'what do we have about X' or 'show me all demos'.
...
@mcp.tool(name="ct_search_content")
...
@mcp.tool(name="ct_update_content")
...
@mcp.tool(name="ct_content_stats")
...
@mcp.tool(name="ct_bulk_register")
async def bulk_register(params: DashboardInput) -> str:
    """Import multiple content pieces at once. Used for initial seed.

    ⚠️ This will overwrite existing content data. Only run on first setup.
    ...
    """
    return "⚠️ Bulk register is handled by ct_seed_data. Use that instead."
...
@mcp.tool(name="ct_update_backlog")
...
@mcp.tool(name="ct_complete_backlog")
...
@mcp.tool(name="ct_track_keyword")
...
@mcp.tool(name="ct_list_keywords")
...
@mcp.tool(name="ct_keyword_wins")
async def keyword_wins(params: DashboardInput) -> str:
    """Show keywords where FixMyCert is performing well.
...
@mcp.tool(name="ct_keyword_snapshot")
...
@mcp.tool(name="ct_log_seo_change")
...
@mcp.tool(name="ct_list_seo_changes")
...
@mcp.tool(name="ct_log_index_submission")
...
@mcp.tool(name="ct_list_index_submissions")
...
@mcp.tool(name="ct_add_partner")
async def add_partner(params: AddPartnerInput) -> str:
    """Add a new partnership lead.
...
@mcp.tool(name="ct_list_partners")
async def list_partners(params: DashboardInput) -> str:
    """List all partnership leads.
    ...
    if not items:
        return "No partnership leads tracked."

    if params.response_format == ResponseFormat.JSON:
        return json.dumps(items, indent=2)

    lines = ["## Partnership Leads\n"]
    for item in items:
...
@mcp.tool(name="ct_update_partner")
...
@mcp.tool(name="ct_dashboard")
async def dashboard(params: DashboardInput) -> str:
    """High-level overview of FixMyCert content status.

    Shows content counts, backlog status, keyword wins, pending items.
    Run this as a weekly check-in alongside yt_dashboard.
    ...
    lines = [
        "# 📊 FixMyCert Content Dashboard\n",
        f"## Content Library: {len(content)} pieces",
        f"- Video coverage: {with_video}/{len(content)} ({round(with_video / len(content) * 100, 1) if content else 0}%)",
    ]
    for t, count in sorted(by_type.items(), key=lambda x: -x[1]):
        lines.append(f"- {t}: {count}")
    ...
    lines.append(f"\n## Partnerships: {len(partners)} leads")
...
@mcp.tool(name="ct_seed_data")
async def seed_data(params: DashboardInput) -> str:
    """Seed the database from the FixMyCert tracker spreadsheet data.
    ...
    # ============================================================
    # CONTENT (210 items from v6 spreadsheet)
    # ============================================================
    content_data = _get_seed_content()
    _save(CONTENT_FILE, content_data)

    # ============================================================
    # BACKLOG (48 items)
    # ============================================================
    backlog_data = _get_seed_backlog()
    _save(BACKLOG_FILE, backlog_data)

    # ============================================================
    # KEYWORDS (10 items)
    # ============================================================
    keywords_data = _get_seed_keywords()
    _save(KEYWORDS_FILE, keywords_data)

    # ============================================================
    # SEO LOG (5 entries)
    # ============================================================
    seo_data = _get_seed_seo_log()
    _save(SEO_LOG_FILE, seo_data)

    # ============================================================
    # INDEX SUBMISSIONS (3 entries)
    # ============================================================
    idx_data = _get_seed_index_submissions()
    _save(INDEX_SUBS_FILE, idx_data)

    # ============================================================
    # PARTNERSHIPS (2 real leads)
    # ============================================================
    partner_data = _get_seed_partnerships()
    _save(PARTNERSHIPS_FILE, partner_data)

    return (
        f"✅ Seed complete!\n"
        f"- Content: {len(content_data)} items\n"
        f"- Backlog: {len(backlog_data)} items\n"
        f"- Keywords: {len(keywords_data)} items\n"
        f"- SEO Log: {len(seo_data)} entries\n"
        f"- Index Submissions: {len(idx_data)} entries\n"
        f"- Partnerships: {len(partner_data)} leads"
    )


# ============================================================
# SEED DATA FUNCTIONS (extracted for readability)
# ============================================================


def _get_seed_partnerships() -> list:
    return [
        {
            "name": "Marlene Dehart",
            "company": "Independent (formerly Oracle Cloud)",
            "relationship": "Former colleague - Director over Oracle Cloud",
            "network": "Enterprise Oracle cloud customers, security/compliance buyers",
            "partnership_type": "enterprise-referral",
            "potential_value": "$25K+ deals",
            "status": "Warm - engaged on LinkedIn",
            "next_action": "",
            "notes": [],
        },
        {
            "name": "Ed Harmoush",
            "company": "Practical Networking",
            "relationship": "Content creator connection",
            "network": "Network engineers, cert students",
            "partnership_type": "content-collaboration",
            "potential_value": "Traffic/credibility",
            "status": "To explore",
            "next_action": "",
            "notes": [],
        },
...
def _get_seed_index_submissions() -> list:
    return [
        {"date": "2026-02-08", "path": "/guides/dns-01-automation", "title": "DNS-01 Automation", "method": "gsc-request", "status": "submitted", "notes": ""},
        {"date": "2026-02-08", "path": "/guides/tls-alpn-01-automation", "title": "TLS-ALPN-01 Automation", "method": "gsc-request", "status": "submitted", "notes": ""},
        {"date": "2026-02-08", "path": "/guides/which-dcv-method-to-automate", "title": "Which DCV Method to Automate", "method": "gsc-request", "status": "submitted", "notes": ""},
    ]


def _get_seed_seo_log() -> list:
    return [
        {"date": "2026-01-25", "page": "/tools/csr-checker", "change_type": "title-meta", "old_value": "CSR Checker", "new_value": "Free CSR Decoder & Checker - Validate Your Certificate Request", "target_keyword": "csr checker", "before_position": ""},
        {"date": "2026-01-25", "page": "/guides/openssl-installation", "change_type": "content-expansion", "old_value": "Basic guide", "new_value": "Added Alpine Linux, macOS/LibreSSL sections", "target_keyword": "install openssl alpine linux", "before_position": ""},
        {"date": "2026-01-26", "page": "/checklists/pci-dss-4-audit-prep", "change_type": "new-content", "old_value": "", "new_value": "", "target_keyword": "pci dss 4.0 certificate requirements", "before_position": ""},
        {"date": "2026-01-26", "page": "LinkedIn", "change_type": "new-content", "old_value": "", "new_value": "Infographic style guide created", "target_keyword": "", "before_position": ""},
        {"date": "2026-02-08", "page": "YouTube - How TLS Works", "change_type": "title-meta", "old_value": "", "new_value": "A/B Thumbnail Test", "target_keyword": "", "before_position": ""},
    ]


def _get_seed_keywords() -> list:
    return [
        {"keyword": "openssl s_client hangs", "impressions": 1, "clicks": 1, "position": 5.0, "ctr": "100%", "status": "winning", "action": "Monitor", "content_path": "/guides/openssl-s-client", "last_updated": "2026-02-08", "history": []},
        {"keyword": "install openssl alpine linux", "impressions": 25, "clicks": 0, "position": 7.24, "ctr": "0%", "status": "optimized", "action": "Added to guide Jan 25", "content_path": "/guides/openssl-installation", "last_updated": "2026-02-08", "history": []},
        {"keyword": "macos default openssl or libressl", "impressions": 12, "clicks": 0, "position": 7.92, "ctr": "0%", "status": "optimized", "action": "Added to guide Jan 25", "content_path": "/guides/openssl-installation", "last_updated": "2026-02-08", "history": []},
        {"keyword": "is openssl pre-installed on macos", "impressions": 15, "clicks": 0, "position": 9.93, "ctr": "0%", "status": "optimized", "action": "Added to guide Jan 25", "content_path": "/guides/openssl-installation", "last_updated": "2026-02-08", "history": []},
        {"keyword": "tls 1.3 handshake diagram gif animated", "impressions": 20, "clicks": 0, "position": 7.25, "ctr": "0%", "status": "winning", "action": "Auto-play exists, GIF export backlog", "content_path": "/demos/tls-handshake", "last_updated": "2026-02-08", "history": []},
        {"keyword": "f5 ssl passthrough", "impressions": 33, "clicks": 0, "position": 19.0, "ctr": "0%", "status": "new-content", "action": "Guide shipped Jan 25", "content_path": "/guides/f5-ssl-passthrough", "last_updated": "2026-02-08", "history": []},
        {"keyword": "csr checker", "impressions": 62, "clicks": 0, "position": 45.0, "ctr": "0%", "status": "optimized", "action": "Meta updated Jan 25", "content_path": "/tools/csr-checker", "last_updated": "2026-02-08", "history": []},
        {"keyword": "f5 ssl", "impressions": 60, "clicks": 0, "position": 29.0, "ctr": "0%", "status": "needs-time", "action": "Existing content", "content_path": "/guides/f5-ssl-profiles", "last_updated": "2026-02-08", "history": []},
        {"keyword": "f5 tls", "impressions": 59, "clicks": 0, "position": 28.0, "ctr": "0%", "status": "needs-time", "action": "Existing content", "content_path": "/guides/f5-ssl-profiles", "last_updated": "2026-02-08", "history": []},
        {"keyword": "python ssl certificate verify failed", "impressions": 50, "clicks": 3, "position": 4.2, "ctr": "6%", "status": "winning", "action": "Crushing it", "content_path": "/guides/python-ssl-certificate-errors", "last_updated": "2026-02-08", "history": []},
    ]


def _get_seed_backlog() -> list:
    raw = [
        ("SSL Labs A+ Checklist", "checklist", "compliance", "high", "High utility, refs existing content", "done", "2026-01-25"),
        ("Caddy SSL Certificate", "guide", "web-servers", "high", "Growing dev popularity", "done", "2026-01-31"),
        ("Azure Key Vault Certificates", "guide", "cloud", "high", "Enterprise Azure shops", "ready", None),
        ("Certificate Expiry Calendar Tool", "tool", "tools", "medium", "ICS generator, practical", "spec-needed", None),
        ("F5 KB K13302 Disgronified", "guide", "f5", "medium", "Chain config, #1 pain point", "ready", None),
        ("Supply Chain Certificate Requirements", "guide", "compliance", "medium", "Enterprise customer demands", "spec-needed", None),
        ("TLS Handshake GIF Export", "demo", "enhancement", "low", "Nice to have, auto-play exists", "backlog", None),
        ("Certificate Revocation Comparison Demo", "demo", "certificates", "low", "CRL vs OCSP vs Stapling visual", "spec-needed", None),
        ("PQC Algorithm Comparison Demo", "demo", "enterprise", "low", "ML-KEM vs ML-DSA vs SLH-DSA", "spec-needed", None),
        ("F5 KB K14783 Client SSL Profile", "guide", "f5", "low", "Foundation KB", "backlog", None),
        ("F5 KB K14806 Server SSL Profile", "guide", "f5", "low", "Companion to K14783", "backlog", None),
        ("F5 KB K14620 Managing Certs via GUI", "guide", "f5", "low", "Everyone's first task", "backlog", None),
        ("F5 KB K17370 Cipher Configuration", "guide", "f5", "low", "Auditor demand", "backlog", None),
        ("PEM to PKCS12 Converter", "tool", "tools", "low", "Browser-based conversion", "backlog", None),
        ("Certificate Chain Validator", "tool", "tools", "low", "", "backlog", None),
        ("Venafi Implementation Checklist", "checklist", "venafi", "low", "", "backlog", None),
        ("ACME/Let's Encrypt Setup Checklist", "checklist", "compliance", "low", "", "backlog", None),
        ("GCP Certificate Manager", "guide", "cloud", "low", "", "backlog", None),
        ("The PKI Automation Gap: Why Nobody Is Ready for 47-Day Certificates", "guide", "compliance", "critical", "Pillar content for automation series - timely with 47-day deadline", "ready", None),
        ("PKI Automation Reality: Discovery", "guide", "compliance", "high", "", "backlog", None),
        ("PKI Automation Reality: Validation", "guide", "compliance", "high", "", "backlog", None),
        ("PKI Automation Reality: Enrollment", "guide", "compliance", "high", "", "backlog", None),
        ("PKI Automation Reality: Deployment", "guide", "compliance", "high", "", "backlog", None),
        ("PKI Automation Reality: Monitoring", "guide", "compliance", "high", "", "backlog", None),
        ("PKI Automation Reality: Renewal", "guide", "compliance", "high", "", "backlog", None),
        ("PKI Automation Reality: Revocation", "guide", "compliance", "high", "", "backlog", None),
        ("PKI Automation Reality: Key Rotation", "guide", "compliance", "high", "", "backlog", None),
        ("PKI Automation Maturity Assessment", "tool", "compliance", "high", "", "spec-needed", None),
        ("PKI Automation Maturity Model", "guide", "compliance", "high", "", "backlog", None),
        ("47-Day Readiness Audit Checklist", "checklist", "compliance", "high", "", "backlog", None),
        ("Automation Gap Finder Checklist", "checklist", "compliance", "medium", "", "backlog", None),
        ("ACME Automation Flow", "demo", "compliance", "medium", "", "backlog", None),
        ("Approval Workflow Visualization", "demo", "compliance", "medium", "", "backlog", None),
        ("When Automated Renewal Failed: Post-Mortems", "guide", "compliance", "medium", "", "backlog", None),
        ("Which DCV Method Should You Automate?", "guide", "compliance", "critical", "Decision flowchart - anchor for DCV automation series, drives to all methods", "spec-complete", None),
        ("How to Automate DNS-01 Validation", "guide", "compliance", "critical", "", "backlog", None),
        ("How to Automate HTTP-01 Validation", "guide", "compliance", "critical", "", "backlog", None),
        ("How to Automate TLS-ALPN-01 Validation", "guide", "compliance", "critical", "", "backlog", None),
        ("ACME Protocol Implementation Guide", "guide", "compliance", "high", "", "backlog", None),
        ("DNS-01 Automation Checklist", "checklist", "compliance", "high", "", "backlog", None),
        ("HTTP-01 Automation Checklist", "checklist", "compliance", "high", "", "backlog", None),
        ("TLS-ALPN-01 Automation Checklist", "checklist", "compliance", "high", "", "backlog", None),
        ("ACME Client Selection Checklist", "checklist", "compliance", "high", "", "backlog", None),
        ("DNS-01 Automation Runbook", "guide", "compliance", "high", "", "backlog", None),
        ("HTTP-01 Automation Runbook", "guide", "compliance", "high", "", "backlog", None),
        ("TLS-ALPN-01 Automation Runbook", "guide", "compliance", "high", "", "backlog", None),
        ("ACME Troubleshooting Runbook", "guide", "compliance", "medium", "", "backlog", None),
        ("DCV Method Decision Tool", "tool", "compliance", "medium", "", "backlog", None),
    ]

    items = []
    for title, ctype, cat, priority, why, status, completed in raw:
        record = {
            "backlog_id": _make_id(title),
            "title": title,
            "content_type": ctype,
            "category": cat,
            "priority": priority,
...
def _get_seed_content() -> list:
    """All 210 content items from the v6 spreadsheet."""
    raw = [
        ("DNS-01 Automation", "guide", "compliance", "/guides/dns-01-automation", "https://www.youtube.com/watch?v=64JHWgrCHpU", "Feb 2026"),
        ("TLS-ALPN-01 Automation", "guide", "compliance", "/guides/tls-alpn-01-automation", "https://www.youtube.com/watch?v=8v9veID3A_Q", "Feb 2026"),
        ("AWS ACM Certificate Manager", "guide", "cdn", "/guides/aws-acm-certificate-manager", "https://www.youtube.com/watch?v=hNFrbqWvZzI", "Jan 2026"),
        ("Cloudflare Error 526", "guide", "cdn", "/guides/cloudflare-error-526", "https://www.youtube.com/watch?v=m0hpo10ZvF4", "Dec 2025"),
        ("Which DCV Method to Automate", "guide", "compliance", "/guides/which-dcv-method-to-automate", "https://www.youtube.com/watch?v=M4T7bVcjUjk", "Feb 2026"),
        ("HTTP-01 Automation", "guide", "compliance", "/guides/http-01-automation", "https://www.youtube.com/watch?v=MMTW4CrcYdE", "Feb 2026"),
        ("47-Day Certificate Timeline", "guide", "compliance", "/guides/47-day-certificate-timeline", "https://www.youtube.com/watch?v=TlGXMTIwU8I", "Jan 2026"),
        ("DCV Methods Sunset", "guide", "compliance", "/guides/dcv-methods-sunset", "https://www.youtube.com/watch?v=wCFnpj4zntU", "Jan 2026"),
    ]

    # I need to extract the remaining ~202 items from the spreadsheet.
    # This seed function will be completed by the ct_seed_from_xlsx tool
    # which reads the actual file. For now, return what we have as a starting structure.
    # The full import happens via the separate seed-from-xlsx script.

    items = []
    for title, ctype, cat, path, yt_url, date_added in raw:
        record = {
            "content_id": _make_id(path),
            "title": title,
            "content_type": ctype,
            "category": cat,
            "path": path,
...
# ============================================================
# SEED FROM XLSX (the real full import)
# ============================================================


@mcp.tool(name="ct_seed_from_xlsx")
async def seed_from_xlsx(params: DashboardInput) -> str:
    """Import all content from the fixmycerttracker xlsx file.

    Reads the spreadsheet from the standard upload location and imports
    all tabs. This is the primary seed method.
    ...
    # Find the xlsx file — prefer most recent version
    xlsx_path = None
    search_dirs = [
        Path.home(),
        Path.home() / "Downloads",
    ...
    for d in search_dirs:
        if d.exists():
            all_matches.extend(d.glob("fixmycerttracker*.xlsx"))

    if all_matches:
        # Sort by modification time, newest first
        all_matches.sort(key=lambda f: f.stat().st_mtime, reverse=True)
        xlsx_path = all_matches[0]

    if not xlsx_path:
        return "❌ Could not find fixmycerttracker*.xlsx. Please specify the path."
    ...
    def _get_sheet(name: str):
        """Find sheet by name, handling unicode/whitespace variants."""
        # Exact match first
        if name in wb.sheetnames:
            return wb[name]
        # Fuzzy match
        for sn in wb.sheetnames:
            cleaned = sn.strip().replace("\xa0", " ")
            if cleaned.lower() == name.lower():
                return wb[sn]
        return None

    # === CONTENT ===
    ws = _get_sheet("Content")
    content = []
    for row in ws.iter_rows(min_row=1, values_only=True):
        title = row[0]
        if not title or title == "Title" or title == "FixMyCert Content Tracker":
            continue
        path = str(row[3] or "").strip()
        content.append({
            "content_id": _make_id(path) if path else _make_id(str(title)),
            "title": str(title).strip(),
            "content_type": str(row[1] or "").strip().lower(),
            "category": str(row[2] or "").strip().lower().replace(" ", "-"),
            "path": path,
            "youtube_url": str(row[4] or "").strip(),
            "status": str(row[6] or "live").strip().lower(),
            "date_added": str(row[5] or "").strip(),
            "tags": [],
            "notes": [str(row[7]).strip()] if len(row) > 7 and row[7] else [],
        })
    _save(CONTENT_FILE, content)
    results.append(f"Content: {len(content)} items")

    # === BACKLOG ===
    ws = _get_sheet("Backlog")
    backlog = []
    for row in ws.iter_rows(min_row=2, values_only=True):
        title = row[0]
        if not title:
            continue
        status_raw = str(row[5] or "backlog").strip()
        completed_date = None
        if "DONE" in status_raw.upper():
            # Extract date from "DONE - Jan 25"
            parts = status_raw.split("-", 1)
            if len(parts) > 1:
                completed_date = parts[1].strip()
            status_raw = "done"
        else:
            # Normalize known status variants
            status_lower = status_raw.lower()
            status_map = {
                "ready to spec": "ready",
                "ready-to-spec": "ready",
                "spec complete": "spec-complete",
                "spec complete - ready for replit": "spec-complete",
                "spec complete---ready for replit": "spec-complete",
            }
            for pattern, normalized in status_map.items():
                if pattern in status_lower.replace("-", " ").replace("  ", " "):
                    status_raw = normalized
                    break
        backlog.append({
            "backlog_id": _make_id(str(title)),
            "title": str(title).strip(),
            "content_type": str(row[1] or "").strip().lower(),
            "category": str(row[2] or "").strip().lower().replace(" ", "-"),
            "priority": str(row[3] or "medium").strip().lower(),
            "why": str(row[4] or "").strip(),
            "status": status_raw.lower().replace(" ", "-"),
            "completed_date": completed_date,
            "notes": [],
        })
    _save(BACKLOG_FILE, backlog)
    results.append(f"Backlog: {len(backlog)} items")

    # === KEYWORDS ===
    ws = _get_sheet("GSC Keywords")
    keywords = []
    for row in ws.iter_rows(min_row=2, values_only=True):
        kw = row[0]
        if not kw:
        ...
        keywords.append({
            ...
            "position": float(row[3]) if row[3] else None,
            "ctr": str(row[4] or "0%").strip(),
            "status": str(row[5] or "").strip().lower().replace(" ", "-"),
            "action": str(row[6] or "").strip(),
            "content_path": str(row[7] or "").strip(),
            "last_updated": _now(),
            "history": [],
        })
    _save(KEYWORDS_FILE, keywords)
    results.append(f"Keywords: {len(keywords)} items")

    # === SEO LOG ===
    ws = _get_sheet("SEO Log")
    seo_log = []
    for row in ws.iter_rows(min_row=2, values_only=True):
        if not row[0]:
            continue
        seo_log.append({
            ...
            "page": str(row[1] or "").strip(),
            "change_type": str(row[2] or "").strip().lower().replace(" + ", "-").replace(" ", "-"),
            "old_value": str(row[3] or "").strip(),
            "new_value": str(row[4] or "").strip(),
            "target_keyword": str(row[5] or "").strip(),
            "before_position": str(row[6] or "").strip(),
        })
    _save(SEO_LOG_FILE, seo_log)
    results.append(f"SEO Log: {len(seo_log)} entries")

    # === INDEX SUBMISSIONS ===
    ws = _get_sheet("Index Submissions")
    idx_subs = []
    for row in ws.iter_rows(min_row=2, values_only=True):
        if not row[0]:
            continue
        idx_subs.append({
            ...
            "method": str(row[3] or "gsc-request").strip().lower().replace(" ", "-"),
            "status": str(row[4] or "submitted").strip().lower(),
            "notes": str(row[6] or "").strip(),
        })
    _save(INDEX_SUBS_FILE, idx_subs)
    results.append(f"Index Submissions: {len(idx_subs)} entries")

    # === PARTNERSHIPS (only real people, skip reference rows) ===
    ws = _get_sheet("Partnership Leads")
    partners = []
    for row in ws.iter_rows(min_row=2, values_only=True):
        name = row[0]
        if not name or "---" in str(name) or "Reference" in str(name):
            continue
        # Skip rows that are clearly descriptions not people
        if str(name).startswith("Enterprise") or str(name).startswith("Sectigo") or str(name).startswith("Content") or str(name).startswith("Consulting"):
            continue
        partners.append({
            "name": str(name).strip(),
            "company": str(row[1] or "").strip(),
            "relationship": str(row[2] or "").strip(),
            "network": str(row[3] or "").strip(),
            "partnership_type": str(row[4] or "").strip().lower().replace(" ", "-"),
            "potential_value": str(row[5] or "").strip(),
            "status": str(row[6] or "").strip(),
            "next_action": str(row[7] or "").strip(),
            "notes": [],
        })
    _save(PARTNERSHIPS_FILE, partners)
    results.append(f"Partnerships: {len(partners)} leads")

    return f"✅ Full import from `{xlsx_path.name}` complete!\n" + "\n".join(f"- {r}" for r in results)
...
if __name__ == "__main__":
    ...
    if _transport == "sse":
        mcp.run(
            ...
            port=int(_os.environ.get("MCP_PORT", "8081")),
        )
    else:
        mcp.run()