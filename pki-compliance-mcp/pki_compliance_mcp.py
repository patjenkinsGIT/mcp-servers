...
EXPLICIT_RELATED_GUIDES = {
    ...
    # keyword heuristic is not reliable here). A dedicated
    # /guides/digicert-global-root-g2 is queued for the 2026-08-18 content
    # batch and is the better target for the DISABLE entry once it ships — it
    # is deliberately NOT listed yet, because it is not registered in the
    # content tracker and the Hub would render a chip to a 404 until then.
    # Append it (keep Root Stores) after ct_get_content resolves it. The
    # NotBefore entry is not that guide's subject and stays on Root Stores.
