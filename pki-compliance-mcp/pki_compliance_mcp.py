...
DEADLINES = [
    ...
    },
    {
        ...
        "date": "2026-08-25",
        "title": "Microsoft disables Baltimore CyberTrust Root and GeoTrust Universal CA; removes Visa Information Delivery Root CA",
        "description": "The August 2026 Trusted Root Program release (Tuesday, August 25, 2026) DISABLES two DigiCert roots outright — Baltimore CyberTrust Root (SHA-1 D4DE20D05E66FC53FE1A50882C78DB2852CAE474) and GeoTrust Universal CA (E621F3354379059A4B68309D8A2F74221587EC79) — and REMOVES Visa Information Delivery Root CA (SHA-256 C57A3ACBE8C06BA1988A83485BF326F2448775379849DE01CA43571AF357E74B). This is a different and harsher mechanism than the NotBefore distrust in the same release: a NotBefore only breaks certificates issued after its date, whereas disabling a root breaks every certificate under it regardless of issuance date. Baltimore CyberTrust Root in particular is a long-lived, heavily embedded root, so the exposure is in places that are slow to update — appliances, IoT and embedded devices, pinned or bundled trust stores, and any Java/OpenSSL trust store maintained separately from the Windows CTL. NOTE on the date: the notice states one NotBefore date (2026-09-15) and lists Disable and Remove as separate actions of the release, without restating a date for them, so the release date is used here.",
        "source": "microsoft",
        "source_url": "https://github.com/TrustedRootProgram/Program-Requirements/blob/main/trusted-root/2026/august-2026.md",
        "category": "certificates",
        "isMajor": True,
        "impact": "Every certificate chaining to Baltimore CyberTrust Root or GeoTrust Universal CA stops validating on Windows — not just new issuance. Inventory before the release, not after.",
        "consequences": {
            "enforcement": "Disabling removes the root's trust in the Windows CTL entirely, so all certificates chaining to it fail validation regardless of when they were issued. This is unlike the NotBefore sets in the same release, where certificates issued before the NotBefore date continue to validate.",
            "scenario": "The failure lands where nobody is looking. A Windows host that has never been touched by the certificate team starts failing TLS to an internal appliance or a partner endpoint whose chain still terminates at Baltimore CyberTrust Root — and because the certificate itself did not change and did not expire, the first hypothesis is a network or firewall problem. Search the estate for chains ending at these two roots before 2026-08-25 rather than diagnosing it afterwards from an outage.",
        },
    },
    {
