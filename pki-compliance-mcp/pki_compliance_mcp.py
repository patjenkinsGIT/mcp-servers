...
DEADLINES = [
    ...
    {
        # DISTINCT from chrome-cpcps-attestation-required above, which is Chrome
        # Root Program Policy v1.8 §1.1.3, Chrome-scoped and already passed
        # (2026-06-15). This one is the CCADB Policy's own obligation and binds
        # every CA Owner in a CCADB-participating root store. Do not merge them.
        "id": "ccadb-policy-cpcps-adherence",
        "date": "2026-09-15",
        "title": "CCADB Policy: CA policy documents must state adherence to the CCADB Policy",
        "description": "CCADB Policy version 2.1 (effective March 20, 2026), section 4 'Policy Disclosures', states verbatim: \"Effective September 15, 2026, CA policy documents MUST state adherence to this CCADB Policy.\" The date is day-precise in the source and is not derived. The obligation reaches every CA Owner whose roots are included in a CCADB-participating root store, so it is broader than the Chrome-scoped CP/CPS attestation that took effect 2026-06-15 — a CA that satisfied the Chrome requirement has not necessarily satisfied this one, because the required statement is adherence to the CCADB Policy itself.",
        "source": "ccadb",
        "source_url": "https://www.ccadb.org/policy",
        "category": "governance",
        "isMajor": False,
        "impact": "CA Owners must have a CP, CPS or combined CP/CPS that explicitly states adherence to the CCADB Policy by 2026-09-15. This is a document-text change, so it lands on the next CP/CPS revision cycle rather than on any certificate.",
        "is_estimated": False,
        "framework_id": "cabforum",
        "jurisdiction": "global",
        "consequences": {
            "enforcement": "From 2026-09-15 a CA Owner's policy documents must state adherence to the CCADB Policy. Non-conformity is a policy-document defect surfaced through CCADB and root-program review rather than something that breaks certificate validation.",
            "scenario": "No action for anyone who does not operate a publicly trusted CA. For those who do, the trap is assuming the Chrome CP/CPS attestation from June already covers this: that one required a statement of adherence to the Chrome Root Program Policy and the CCADB Policy for Chrome participants, whereas this one is the CCADB Policy binding CA Owners across every participating root store. Check the actual sentence in the CP/CPS names the CCADB Policy, and schedule the revision — CP/CPS updates carry their own review and publication lead time, so 24 days is tighter than it looks.",
        },
    },
    {
    ...
    },
    {
        ...
        "consequences": {
            ...
            "scenario": "The failure lands where nobody is looking, and it is GeoTrust. A Windows host that has never been touched by the certificate team starts failing TLS to an internal appliance or a partner endpoint whose chain still terminates at GeoTrust Universal CA — and because the certificate itself did not change and did not expire, the first hypothesis is a network or firewall problem. Search the estate for chains ending at GeoTrust Universal CA before 2026-08-27 rather than diagnosing it afterwards from an outage. Do not spend the same effort on Baltimore CyberTrust Root: anything chaining to it broke in May 2025 when the root expired, so a Baltimore hit found now is an already-broken path or a stale trust-store copy, not a pending one — worth cleaning up, not worth a change freeze.",
        },
    },
    {
    ...
    {
        "id": "digicert-g5-default-issuance",
        "date": "2026-10-15",
        "title": "DigiCert defaults public TLS issuance to G5 root hierarchies",
        "description": "DigiCert ALERT86 (last modified 2026-09-01): on October 15, 2026 DigiCert starts issuing public TLS certificates from the DigiCert TLS RSA4096 Root G5 and DigiCert TLS ECC P384 Root G5 root hierarchies by default. Applies to new orders, renewals, reissues and duplicates; DV, OV and EV; across all DigiCert brands (DigiCert, GeoTrust, Thawte, RapidSSL, Encryption Everywhere). DigiCert's stated reason is the Chrome Root Store two-roots-per-CA-Owner cap from 2027-09-15, after which Chrome supports only DigiCert's two G5 TLS hierarchies. Customers with mission-critical dependencies on DigiCert Global Root G2 (RSA) or G3 (ECC) can ask DigiCert to keep issuing from their current hierarchy during the transition. The same alert also sets 2027-10-15 as the start of ~365-day TLS intermediate CA rotation, when default issuance moves to new G5 RSA and G5 ECC intermediates.",
        "source": "chrome",
        "source_url": "https://knowledge.digicert.com/alerts/digicert-root-strategy-aligning-with-industry-standards",
        "category": "root-store",
        "isMajor": True,
        "is_estimated": False,
        "impact": "The first renewal or reissue after 2026-10-15 chains to a G5 root instead of Global Root G2/G3. Installing the chain DigiCert supplies needs no action; hard-coded G2/G3 chain expectations and pinned intermediates break.",
        "consequences": {
            "enforcement": "DigiCert changes its default issuing hierarchy on 2026-10-15 for every public TLS certificate it issues under any of its brands; existing certificates keep their current chain until renewed. Staying on G2/G3 requires an account-level opt-in arranged with DigiCert.",
            "scenario": "Nothing fails on the date itself. The first DigiCert renewal after it lands on a G5 chain, and anything that pins the old intermediate or bundles only Global Root G2/G3 (a mobile app, an appliance trust store, a partner's mTLS allowlist) rejects a certificate that is otherwise valid. Because the renewal is routine, it gets diagnosed as a bad certificate. Inventory pins and bundled trust stores against the G5 chain before your first post-October renewal.",
        },
    },
    {
...
EXPLICIT_RELATED_GUIDES = {
    ...
    # Both halves of the August 2026 Microsoft release. Same Root Stores guide
    # as the other distrust cards ("certificates" has no default, and the
    # keyword heuristic is not reliable here).
    #
    # The DISABLE entry also carries the dedicated
    # /guides/digicert-global-root-g2 guide, appended 2026-08-25 once BOTH
    # gates passed: the guide resolves in the content tracker (status live,
    # no video) AND https://fixmycert.com/guides/digicert-global-root-g2
    # returns 200. Until 2026-08-18 it did neither, which is why the chip was
    # deferred — an unshipped guide renders a chip to a 404 on the Hub.
    #
    # NOTE the list is REBUILT, not appended to in place:
    # CATEGORY_RELATED_GUIDES["root-store"] is a shared list object referenced
    # by the four Entrust distrust keys, the NotBefore key below and the
    # category default itself, so `.append()` here would silently add this
    # chip to all of them.
    #
    # The NotBefore entry is not that guide's subject and stays on Root
    # Stores alone — decided 2026-08-15, unchanged.
    "microsoft-august-2026-root-disable": CATEGORY_RELATED_GUIDES["root-store"] + [
        {"title": "DigiCert Global Root G2: How to Verify Your Chain Migrated", "url": "/guides/digicert-global-root-g2", "hasVideo": False},
    ],
    "microsoft-august-2026-ctl-notbefore": CATEGORY_RELATED_GUIDES["root-store"],
    "digicert-g5-default-issuance": CATEGORY_RELATED_GUIDES["root-store"] + [
        {"title": "DigiCert Global Root G2: How to Verify Your Chain Migrated", "url": "/guides/digicert-global-root-g2", "hasVideo": False},
    ],
}
