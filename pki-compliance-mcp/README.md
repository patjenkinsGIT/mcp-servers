...
- **Status is computed, never stored** — `get_all_deadlines_unified()` derives `status` from the date on every request (`upcoming`/`passed`). Do **not** hardcode `"status"` on deadline entries; it will drift as dates pass. The only exception is `"status": "ongoing"`, which is preserved — set it on **type (b)** entries: the date started a rule whose failure mode recurs per-reader after that date (renewal traps like NotBefore distrusts, rolling processes, in-force regimes). Type (a) entries (one-time transitions, due dates) take no status. The classification is editorial, documented above `DEADLINES`, locked by `ONGOING_IDS` in `test_source_url_and_estimates.py`, and applies identically to framework sub-list deadlines. Front-end renders `ongoing` as an amber "In force" badge — mapping in `FRONTEND_ONGOING_STATUS_SPEC.md` (Replit change, pending as of 2026-07-21). `ballotStatus: "passed"` means the *ballot* passed voting — the deadline itself stays `upcoming` until its effective date.
...
4. **Dedup** (`dedup_changes`) — drops proposals already in `DEADLINES` (by id or normalized title+date signature), already-current document versions, and previously rejected ids. Review flags additionally dedup against the last 14 days of pending files by anchor signature (ballot codes, regulation names — model-generated ids are not stable day to day), so a held topic appears in the email once per 14 days, not daily. A few anchors are **dominant** (`_DOMINANT_ANCHORS`, currently `cjeu`): they identify a topic on their own and the co-occurring regulation anchors are dropped from the signature. Without that, a CJEU referral bucketed as `cjeu+nis2+transposition` one day and `cjeu+dora+nis2+transposition` the next, so one story recurred under four ids (2026-07-13/21/29/31). Dominant anchors affect the dedup signature only — hold matching keeps the full anchor set — and `load_rejected()` re-collapses stored signatures so pre-existing rejections keep matching after the rule changes.
...
Hold tokens are ballot/regulation anchors (`sc101`, `nis2`, `cjeu`) **plus the tracked CA/B Forum documents an item names** (`evguidelines`, `tlsbr`, `smimebr`, `codesigningbr`, `netsec`). Document tokens match both the id a proposal uses (`ev-guidelines`) and the prose name a flag uses ("EV Guidelines", "S/MIME Baseline Requirements"), so a flag titled *"EV Guidelines Version Discrepancy (v2.0.3 vs v2.0.2)"* holds a `[doc bump] ev-guidelines` proposal even though the two share no ballot code. (2026-07-29 and 07-31: that bump auto-applied under exactly that open flag — the same failure class as the SC0101v2 incident, benign only because the version happened to be right.) Document tokens gate **document version bumps only**; a new deadline that merely cites a document is a different claim and stays on the narrower ballot-anchor gate.

Manual-hold conventions:
- Keys are canonicalized like flag anchors, so `SC0101v2`, `sc101v2`, and `sc101` are the same hold; a document id (`ev-guidelines`) canonicalizes onto its document token.
